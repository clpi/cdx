...
linux_cfg() {
    ...
    LLVM_V=$($LLVM_CONFIG --version)
    MIN_LLVM_VERSION=11.0.0

    if [[ $(version $LLVM_V) -lt $(version $MIN_LLVM_VERSION) ]]; then
        echo "Tried to use  $(which $LLVM_CONFIG) version $LLVM_V"
...
build() {
    ...
    set -x
    $CXX $SRC $DISABLED_WARNINGS $CXXFLAGS $XFLAGS $LDFLAGS -o $OUT
    set -x
...
if [[ $# -eq 1 ]]; then
    case $1 in 
        r| run) build debug; run ;;
        b| build) build ;;
        i|init) build debug; run ;;
        *) build; debug; run ;;
    esac
fi
